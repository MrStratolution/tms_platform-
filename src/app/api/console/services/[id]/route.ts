...
function rowJson(row: typeof cmsServices.$inferSelect) {
...
export async function GET(request: Request, ctx: RouteContext) {
  ...
  return NextResponse.json({ ok: true, service: rowJson(rows[0]) })
}
...
export async function PATCH(request: Request, ctx: RouteContext) {
  ...
  const db = getCustomDb()
  if (!db) return NextResponse.json({ error: 'Database not configured' }, { status: 503 })
  const existing = await db.select().from(cmsServices).where(eq(cmsServices.id, id)).limit(1)
  if (!existing[0]) return NextResponse.json({ error: 'Not found' }, { status: 404 })

  const update: Partial<typeof cmsServices.$inferInsert> = { updatedAt: new Date() }
  ...
  if (parsed.data.promise !== undefined) update.promise = parsed.data.promise
  if (Object.prototype.hasOwnProperty.call(parsed.data, 'proof')) update.proof = parsed.data.proof ?? null
  if (parsed.data.active !== undefined) update.active = parsed.data.active

  await db.update(cmsServices).set(update).where(eq(cmsServices.id, id))
  const rows = await db.select().from(cmsServices).where(eq(cmsServices.id, id)).limit(1)
  return NextResponse.json({ ok: true, service: rowJson(rows[0]!) })
}
...
export async function DELETE(request: Request, ctx: RouteContext) {
  ...
  const db = getCustomDb()
  if (!db) return NextResponse.json({ error: 'Database not configured' }, { status: 503 })
  await db.delete(cmsServices).where(eq(cmsServices.id, id))
