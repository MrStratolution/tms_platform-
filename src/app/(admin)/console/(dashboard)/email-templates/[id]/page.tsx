import type { Metadata } from 'next'
import Link from 'next/link'
import { notFound } from 'next/navigation'
import { eq } from 'drizzle-orm'

import { ConsoleEmailTemplateEditor } from '@/components/console/ConsoleEmailTemplateEditor'
import { getCustomDb } from '@/db/client'
import { cmsEmailTemplates } from '@/db/schema'
import { consoleUserCanEditCustomCss, consoleUserCanWriteContent } from '@/lib/console/rbac'
import { requireConsoleSession } from '@/lib/console/requireConsoleSession'

type Props = { params: Promise<{ id: string }> }

export async function generateMetadata(props: Props): Promise<Metadata> {
  const { id } = await props.params
  return { title: `Email template ${id}` }
}

export default async function ConsoleEmailTemplateEditPage(props: Props) {
  const session = await requireConsoleSession()
  const id = Number.parseInt((await props.params).id, 10)
  if (!Number.isFinite(id) || id < 1) notFound()

  const db = getCustomDb()
  if (!db) {
    return (
      <main className="tma-console-main wide">
        <p className="tma-console-lead tma-console-lead--error" role="alert">
          Database is not configured.
        </p>
      </main>
    )
  }

  const rows = await db.select().from(cmsEmailTemplates).where(eq(cmsEmailTemplates.id, id)).limit(1)
  const row = rows[0]
  if (!row) notFound()

  const canEdit = consoleUserCanWriteContent(session.role)
  const canAdvanced = consoleUserCanEditCustomCss(session.role)

  return (
    <main className="tma-console-main wide">
      <p className="tma-console-back">
        <Link href="/console/email-templates">← All email templates</Link>
      </p>
      <h1 className="tma-console-page-title">Email template</h1>
      <ConsoleEmailTemplateEditor
        id={row.id}
        slug={row.slug}
        initialName={row.name}
        initialSubject={row.subject}
        initialBody={row.body}
        canEdit={canEdit}
        canAdvanced={canAdvanced}
      />
    </main>
  )
}
